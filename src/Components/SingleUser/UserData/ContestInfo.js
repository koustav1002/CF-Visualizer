...
const ContestInfo = ({ userContests }) => {
  ...
  const bestRank = Math.min.apply(
    null,
    userContests.map((data) => {
      return data.rank;
    })
  );

  const worstRank = Math.max.apply(
    null,
    userContests.map((data) => {
      return data.rank;
    })
  ...
  const maxDown = Math.min.apply(
    null,
    userContests.map((data) => {
      return Math.min(0, data.newRating - data.oldRating);
    })
  );
